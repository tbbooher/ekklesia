...
LEGISLATORS_IN = [540,"true"]
LEGISLATORS_FIELDS = "bioguide_id,aliases,district,twitter_id,campaign_twitter_ids,youtube_id,facebook_id,terms"


def get_basic_legislators_info(url, key)
  in_office_counter = 1
  out_office_counter = 1
  all_legislators = []
  (LEGISLATORS_IN[0]/50+1).times do
    p "loading basic in"
    call = JSON.parse(open(url + "/legislators?apikey=#{key}&page=#{in_office_counter}&per_page=50&in_office=#{LEGISLATORS_IN[1]}").read)["results"]
    call.each do |legi|
      p legi["first_name"]
...
def get_more_legislators_info(url, key, fields)
  in_office_counter = 1
  out_office_counter = 1
  all_legislators = []
  (LEGISLATORS_IN[0]/50+1).times do
    p "loading detail in"
    call = JSON.parse(open(url + "/legislators?apikey=#{key}&page=#{in_office_counter}&per_page=50&in_office=#{LEGISLATORS_IN[1]}&fields=#{fields}").read)["results"]
    call.each do |legi|
      all_legislators << legi
    end
    sleep 1
...
def insert_basic_politicians
  get_basic_legislators_info(API, API_KEY).each do |l|
...
def insert_details
  get_more_legislators_info(API, API_KEY, LEGISLATORS_FIELDS).each do |l|
