module AlgorithmHelper
  ...
  def self.calculate_issue_score(l_id, i_id)
    total_score = 0
  ...
  def self.set_issue_score_for_legislators
    Issue.all.each do |issue|
      Legislator.all.each do |legislator|
        p LegislatorIssue.create(legislator_id: legislator.id, issue_id: issue.id, issue_score: self.calculate_issue_score(legislator.id, issue.id))
      end
    end
  end
